...
func main() {
	...
	t0 = time.Now()
	encryptedFeatures := fraud_model_lattigo.Cc_fraud__encrypt__arg0(evaluator, params, ecd, encryptor, features)
	fmt.Printf("  Took %v\n", time.Since(t0))
	...
	t0 = time.Now()
	ctZero1 := fraud_model_lattigo.Cc_fraud__encrypt__zero__0(evaluator, params, ecd, encryptor)
	ctZero2 := fraud_model_lattigo.Cc_fraud__encrypt__zero__1(evaluator, params, ecd, encryptor)
	...
	t0 = time.Now()
	decryptedLogits := fraud_model_lattigo.Cc_fraud__decrypt__result0(evaluator, params, ecd, decryptor, encryptedOutput)
	fmt.Printf("  Took %v\n", time.Since(t0))
