...
from transpiler.examples.hangman import hangman_fhe_py_lib
...
def run(cleartext_input):
  ...
  cleartext_x = int_to_bit_slice(cleartext_input, width=8)
  ...
  result_ciphertext = hangman_fhe_py_lib.hangman_make_move(
      ciphertext_x,
      sks,
      boolean_params,
  )
  ...
  print(f'input {chr(cleartext_input)} --> {bin(result)}')
