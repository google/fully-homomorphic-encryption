...
TEST_F(OpenFheArrayOfStructsTest, DynamicOneDimArrayAssignViaRef) {
  ...
  a.SetEncrypted('a', sk());
  CHECK_EQ(a.Decrypt(sk()), 'a');
  OpenFheRef<unsigned char> a_ref = a;
  CHECK_EQ(a_ref.Decrypt(sk()), 'a');

  OpenFhe<unsigned char> b(cc());
  b.SetEncrypted('b', sk());
  CHECK_EQ(b.Decrypt(sk()), 'b');
  OpenFheRef<unsigned char> b_ref = b;
  CHECK_EQ(b_ref.Decrypt(sk()), 'b');
  ...
  CHECK_EQ(c_dyn_one_dim[0].Decrypt(sk()), 'a');
  CHECK_EQ(c_dyn_one_dim_ref[0].Decrypt(sk()), 'a');
  CHECK_EQ(c_dyn_one_dim[1].Decrypt(sk()), 'b');
  CHECK_EQ(c_dyn_one_dim_ref[1].Decrypt(sk()), 'b');
}
...
TEST_F(OpenFheArrayOfStructsTest, FixedLenOneDimArrayAssignViaRef) {
  ...
  CHECK_EQ(c_fixed_len_one_dim[0].Decrypt(sk()), 'a');
  CHECK_EQ(c_fixed_len_one_dim_ref[0].Decrypt(sk()), 'a');
  CHECK_EQ(c_fixed_len_one_dim[1].Decrypt(sk()), 'b');
  CHECK_EQ(c_fixed_len_one_dim_ref[1].Decrypt(sk()), 'b');
}
...
TEST_F(OpenFheArrayOfStructsTest, FixedLenTwoDimArrayAssignViaRef) {
  ...
  CHECK_EQ(i_fixed_len_two_dim[0].Decrypt(sk())[0], 12);
  CHECK_EQ(i_fixed_len_two_dim[0].Decrypt(sk())[1], 34);
  CHECK_EQ(i_fixed_len_two_dim[1].Decrypt(sk())[0], 56);
  CHECK_EQ(i_fixed_len_two_dim[1].Decrypt(sk())[1], 78);
  ...
  CHECK_EQ(i_fixed_len_two_dim[0].Decrypt(sk())[0], 21);
  CHECK_EQ(i_fixed_len_two_dim[0].Decrypt(sk())[1], 43);
  CHECK_EQ(i_fixed_len_two_dim[1].Decrypt(sk())[0], 65);
  CHECK_EQ(i_fixed_len_two_dim[1].Decrypt(sk())[1], 87);

  i_fixed_len_two_dim[0] = encoded_new_row_1;
  i_fixed_len_two_dim[1] = encoded_new_row_0;

  CHECK_EQ(i_fixed_len_two_dim[1].Decrypt(sk())[0], 21);
  CHECK_EQ(i_fixed_len_two_dim[0].Decrypt(sk())[0], 65);
  CHECK_EQ(i_fixed_len_two_dim[0].Decrypt(sk())[1], 87);
  CHECK_EQ(i_fixed_len_two_dim[1].Decrypt(sk())[1], 43);
}
